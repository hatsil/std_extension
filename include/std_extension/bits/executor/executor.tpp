...
[[nodiscard]] std::future<invocable_result_t<F, Args...>> executor::emplace_back(F &&f, Args &&...args) {
...
template <class... Args, single_use_bindable<Args...> F>
[[nodiscard]] std::future<callable_result_t<F>> executor::emplace_front(F &&f, Args &&...args) {
...
template <bool DiscardFuture, executor::EmplaceAt position, class... Args, bindable<Args...> F>
std::future<callable_result_t<F>> executor::emplace(F &&f, Args &&...args) {
    long expected = 0;
    ...
    auto task = detail::pack(std::forward<F>(f), std::forward<Args>(args)...);
    std::future<callable_result_t<F>> future;
