...
constexpr void destroy_at(E *element) {
    ...
    if constexpr (std::is_array_v<E>) {
        ...
        for (auto it = array->rbegin(); it != array->rend(); ++it) {
            stdext::destroy_at(std::addressof(*it));
        }
...
template <class E>
constexpr allocator<E>::size_type allocator<E>::max_size() noexcept {
    return std::numeric_limits<size_type>::max() / sizeof(E);
}

template <class E>
[[nodiscard]] constexpr E *allocator<E>::allocate(size_type n) {
...
template <class E>
constexpr void allocator<E>::deallocate(E *p, size_type n) {
    if constexpr (alignof(E) > __STDCPP_DEFAULT_NEW_ALIGNMENT__) {
...
constexpr void allocator<E>::construct(U *p, Args &&...args) {
    stdext::construct_at(p, std::forward<Args>(args)...);
}
...
template <class E>
class allocator<const volatile E>
{
public:
    using value_type = E;
